﻿﻿﻿(function() {
  ...
  let isProgrammaticChange = false;
	...
	const config = {
      // kViewer API URL (App200)
      KVIEWER_API_URL: 'https://9634221e.viewer.kintoneapp.com/public/api/records/5236d36cc13bb6a3c61fcea761caa6e258500fbfe1a2557b64f5deb46577685c/1',
  ...
  function switchStep(step) {
    ...
    const form = document.querySelector('.formrun-form, form');
    if (form) {
        form.classList.remove('gemini-step-1', 'gemini-step-2');
        form.classList.add(`gemini-step-${step}`);
    }
    ...
    if (showMainForm) {
      if (submitButton) {
        const btnContainer = submitButton.parentNode;
        if (btnContainer) {
          const outerContainer = btnContainer.parentNode;
          if (outerContainer) {
            outerContainer.style.width = '';
            outerContainer.style.textAlign = '';
          }
        }
      }

      const submitFlagInput = document.querySelector(`[data-vv-name="${config.fbFields.HIDDEN_SUBMIT_FLAG}"]`);
      if (submitFlagInput) {
        const fieldWrapper = submitFlagInput.closest('.column.el-form-item.field');
        if (fieldWrapper) {
  ...
  function updateNavigationButtons() {
      ...
      if (config.state.currentStep === 1) {
              ...
              if (isWishDateValid && isFormValid) {
                  ...
                  if (!isWishDateValid) {
                      ...
                      if (wishDatesArea) {
                       ...
                       if (firstInvalid) {
                          firstInvalid.focus({ behavior: 'smooth', block: 'center' });
          ...
          try {
              ...
              console.warn('[Gemini] 送信フラグの処理に失敗', e);
          }

          const submitButton = document.querySelector('.fb-submit');
          if (submitButton) {
              const oldBackBtn = submitButton.parentNode.querySelector('.gemini-injected-back-btn');
              if (!oldBackBtn) {
                  ...
                  submitButton.parentNode.insertBefore(backBtn, submitButton.nextSibling);
              }
...
function getReservationSummaryHtml() {
      const rec = config.state.fbState.record;
      let html = '';

      const requirement = rec[config.fbFields.REQUIREMENT]?.value || '';
      ...
      if (requirement === '初診') {
          html += createSummaryRow('紹介元医療機関名', rec[config.fbFields.REFERRAL_HOSPITAL]?.value);
          html += createSummaryRow('紹介元医療機関電話番号', rec[config.fbFields.REFERRAL_TEL]?.value);
          html += createSummaryRow('持参画像CD', rec[config.fbFields.REFERRAL_CD]?.value);
      }

      if (requirement === '変更' || requirement === '取消') {
        html += createSummaryRow('現在の予約日時', rec[config.fbFields.FIXED_DATETIME]?.value);
      }

      if (requirement !== '取消') {
        const combinedShinryo = rec[config.fbFields.DEPARTMENT_COMBINED]?.value || '';
        html += createSummaryRow('診療科', combinedShinryo);
        html += createSummaryRow('担当医師', rec[config.fbFields.DOCTOR]?.value);
        
        const yoyakuMethod = rec[config.fbFields.YOYAKU_METHOD]?.value || '';
        ...
        if(yoyakuMethod === config.YOYAKU_METHOD_SPECIFIC) {
            ...
            for(let i = 1; i <= config.MAX_WISH_DATES; i++){
                const wishValue = rec[config.fbFields['WISH_' + i]]?.value;
                if(wishValue) {
            ...
            if(hasWish) html += createSummaryRow('希望日時', `_WISH_HTML_${wishHtml}`);
             ...
             html += createSummaryRow('希望時間帯', rec[config.fbFields.WISH_TIME_OMAKASE]?.value);
        }
      }
  ...
  function showFinalSummary() {
      ...
      const requirement = rec[config.fbFields.REQUIREMENT]?.value || '';
      if (requirement === '初診') {
          html += createSummaryRow('症状', rec[config.fbFields.SYMPTOM]?.value);
      } else if (requirement === '変更' || requirement === '取消') {
          const reasonLabel = `${requirement}理由`;
          html += createSummaryRow(reasonLabel, rec[config.fbFields.REASON]?.value);
      }
      
      html += createSummaryRow('申込者', rec[config.fbFields.APPLICANT].value);
      html += createSummaryRow('申込者補足', rec[config.fbFields.APPLICANT_SUPPLEMENT].value);
      html += createSummaryRow('カルテNo', rec[config.fbFields.CHART_NO].value);
      const name = `${rec[config.fbFields.LAST_NAME_KANJI].value} ${rec[config.fbFields.FIRST_NAME_KANJI].value}`;
      html += createSummaryRow('お名前（漢字）', name);
      const kana = `${rec[config.fbFields.LAST_NAME_KANA].value} ${rec[config.fbFields.FIRST_NAME_KANA].value}`;
      html += createSummaryRow('お名前（ふりがな）', kana);
      html += createSummaryRow('生年月日', rec[config.fbFields.DOB].value);
      html += createSummaryRow('性別', rec[config.fbFields.GENDER].value);
      html += createSummaryRow('郵便番号', rec[config.fbFields.POSTAL_CODE].value);
      
      const address = [
          rec[config.fbFields.ADDRESS].value,
          rec[config.fbFields.STREET].value,
          rec[config.fbFields.BUILDING].value
      ].filter(Boolean).join(' ');
      html += createSummaryRow('住所', address);
      
      html += createSummaryRow('電話番号①', rec[config.fbFields.TEL1].value);
      html += createSummaryRow('電話番号②', rec[config.fbFields.TEL2].value);
      html += createSummaryRow('連絡時間帯', rec[config.fbFields.CONTACT_TIME].value);
      html += createSummaryRow('メールアドレス', rec[config.fbFields.EMAIL].value);
      html += createSummaryRow('その他何かありましたらご記入ください', rec[config.fbFields.OTHER_NOTES].value);
      
      html += createSummaryRow('個人情報について', rec[config.fbFields.PRIVACY_AGREE].value);
  ...
  async function fetchKintoneData() {
      ...
      const jsonString = commonRecord['設定情報']?.value;
      console.log('[Gemini] JSON string length:', jsonString ? jsonString.length : 0);
      ...
      console.log('[Gemini] Parsed records count:', config.state.kintoneRecords.length);
      console.log('[Gemini] Descriptions:', config.state.descriptions);
  ...
  async function fetchAddress(postalCode) {
    try {
      ...
      console.error('[郵便番号検索] 検索処理中にエラーが発生しました:', error);
      return null;
    }
  }
  ...
  function updateFbField(fieldCode, value) {
      if (config.state.fbState && config.state.fbState.record[fieldCode]) {
          config.state.fbState.record[fieldCode].value = value;
      } else {
          console.warn(`[Gemini-Debug] WARN: フィールド「${fieldCode}」がFormBridgeのstate内に見つかりません。`);
      }
  }
  ...
  function checkStep1AndToggleNav() {
  }
  ...
  function getFilteredRecords() {
      ...
      // 2. 診療科ごとのステータスチェック (descriptions)
 ...
 function createRequirementSection() {
      ...
      radioContainer.addEventListener('change', (e) => {
          ...
          const fixedDateSelects = document.querySelectorAll('.g-fixed-resv-container select');
          fixedDateSelects.forEach(sel => sel.required = false);
...
function updateReasonSection(requirement) {
  ...
  function createFixedResvPullDown() {
      ...
      const updateFixedDateOutput = () => {
          ...
          if (config.state.requirement === '取消') {
              const hasValue = !!config.state.fbState.record[config.fbFields.FIXED_DATETIME]?.value;
              if (hasValue) {
                  // This was startPatientForm();
              }
          }
  ...
  async function handleAddressSearch() {
    ...
    try {
        ...
        console.error('[住所検索] ハンドラ内で予期せぬエラーが発生しました。', error);
        if(errorElement) errorElement.textContent = '住所の検索中にエラーが発生しました。';
    }
  }
...
function injectStyles() {
      ...
      const css = `
        ...
        .g-name-fields > .g-form-control, .g-dob-fields > .g-form-control { 
            flex: 1; 
        }
        ...
        .g-multistage-container, 
        .g-fixed-resv-container {
            display: flex;
            flex-wrap: wrap;
            gap: 10px;
        }

        .g-multistage-container > div,
        .g-fixed-resv-container > div {
            flex: 1;
            max-width: 300px;
            min-width: 120px;
        }
        
        .g-multistage-container > div .g-form-control,
        .g-fixed-resv-container > div .g-form-control {
            width: 100%;
            max-width: none;
        }
        ...
        .g-wish-container > label {
            flex-shrink: 0;
            font-weight: bold;
            margin-bottom: 0;
        }
        .g-wish-container > .g-form-control {
            flex-grow: 1;
            flex-shrink: 1;
            width: auto;
            min-width: 240px;
        ...
        form.gemini-step-2 .fb-submit {
            display: inline-flex !important;
        }

        .gemini-nav-btn:hover,
        form .fb-submit:hover {
        ...
        .gemini-nav-btn.gemini-btn-primary {
            background-color: #007bff !important;
            border-color: #007bff !important;
            color: white !important;
        }
        
        .gemini-nav-btn.gemini-injected-back-btn {
            background-color: #6c757d !important;
            border-color: #6c757d !important;
            color: white !important;
        }
        
        form .fb-submit + .gemini-injected-back-btn {
        ...
        @media (max-width: 768px) {
            ...
            .g-address-container > div,
            .g-multistage-container > div,
            .g-multistage-container .g-form-control,
            .g-fixed-resv-container > div,
            .g-wish-container > .g-form-control,
            .g-name-fields > .g-form-control, 
            .g-dob-fields > .g-form-control {
                width: 100%;
                max-width: none;
            }
        }
  ...
  /****************************************************************
   * FormBridge イベントハンドラ (起動処理)
   ****************************************************************/
  fb.events.form.mounted = [async function(state) {
      console.log('[Gemini] fb.events.form.mounted triggered');
      config.state.fbState = state;
      
      const form = document.querySelector('.formrun-form, form');
      if(!form) {
          console.error('[FATAL] 初期化失敗: フォーム要素が見つかりませんでした。');
          return state;
      }
      ...
      try {
          ...
          form.prepend(navigationContainer);
          form.prepend(summaryContainer);
          form.prepend(patientFormContainer);
          form.prepend(wizardContainer);
          
          injectStyles();
          console.log('[Gemini] Starting fetchData...');
          await fetchData();
          console.log('[Gemini] fetchData completed.');
          ...
          setTimeout(() => {
            console.log('[Gemini] Switching to step 1 and showing form...');
          ...
          console.error('[FATAL] 初期化処理中に致命的なエラーが発生しました。', error);
          wizardContainer.innerHTML = `<p style="color: red;">エラー: 予約システムの読み込みに失敗しました。ページを再読み込みしてください。</p><p style-="font-size:12px;">エラー詳細: ${error.message}</p>`;
