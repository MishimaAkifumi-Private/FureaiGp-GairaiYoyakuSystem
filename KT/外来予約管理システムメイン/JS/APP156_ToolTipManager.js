(function() {
  ...
  const TOOLTIP_TEXT_MAP = {
    ...
    '掲載': '患者が閲覧する予約Webフォーム上での当医師の掲載・予約受付の制御設定です。\n・通常表示: 通常通りWebフォームで予約受付を行います。\n・非表示: Webフォームの選択肢から除外します。\n・空枠無し: フォーム上の予約枠をすべて意図的にグレーアウト（空き枠なし）として表示します。\n・電話誘導: フォーム上で希望日時選択を表示せず、電話誘導メッセージを表示して電話での予約を案内します。',
    '予約受付': '患者が閲覧する予約Webフォーム上での当医師の掲載・予約受付の制御設定です。\n・通常表示: 通常通りWebフォームで予約受付を行います。\n・非表示: Webフォームの選択肢から除外します。\n・空枠無し: フォーム上の予約枠をすべて意図的にグレーアウト（空き枠なし）として表示します。\n・電話誘導: フォーム上で希望日時選択を表示せず、電話誘導メッセージを表示して電話での予約を案内します。',
    '電話誘導メッセージ': '「掲載」を「電話誘導」に設定した場合に、予約Webフォーム上で患者に表示される案内メッセージです（例：「別途お電話にてお問い合わせください」）。',
