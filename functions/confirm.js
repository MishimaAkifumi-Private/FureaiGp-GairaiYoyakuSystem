    ...
    switch (data.type) {
      ...
      case "取消":
        ...
        htmlBody = `
          ...
          <p>本メールは手続き完了の通知のみとなります。<br>別途お手続きは不要です。<br><br>お大事になさってください。</p>
          ${footerHtml}
        `;
        break;
...
exports.confirmReservation = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (!mailReadDate) {
        ...
        const keepStatusList = ["閲覧期限切れ", "要電話対応", "電話合意済", "完了", "メール既読", "メール合意済"];
    ...
    const html = getConfirmedHtml(record, recordId, showCancel, null, centerName, phoneNumber);
 ...
 * キャンセル完了時のHTML
 */
function getCancellationCompletedHtml(centerName, phoneNumber) {
    return `
      <!DOCTYPE html>
      <html lang="ja">
      <head>
